 ...
 * (with 'load', 'save' and 'add' buttons and a search field) and a status bar
 * that displays the current time.
 *
 * The table actually displays a filtered subset of the timetable entries,
 * according to which entries match the search field.
 */
public class MainUI {
    ...
    public void display() {
        ...
        var loadBtn = new Button(bundle.getString("Load_Button"));
        var saveBtn = new Button(bundle.getString("Save_Button"));
        var addBtn = new Button(bundle.getString("Add_Button"));
        var filterText = new TextField();
        filterText.setPromptText("Search");
        ToolBar toolBar = new ToolBar(loadBtn, saveBtn, addBtn, new Separator(), filterText);
        ...
        filterText.textProperty()
                .addListener((field, oldVal, newVal) -> filteredEntries.setPredicate(entry -> matches(entry, newVal)));

        // Table columns
        TableColumn<TimetableEntry, String> routeIdCol = new TableColumn<>(bundle.getString("Route_Id_Col"));
        ...
        entryTable.getColumns()
                .setAll(List.of(routeIdCol, fromCol, destinationCol, departureTimeCol, arrivalTimeCol, durationCol));

        // Formatting table column values
        routeIdCol.setCellValueFactory((cell) -> new SimpleStringProperty(cell.getValue().getRouteId()));

        fromCol.setCellValueFactory((cell) -> new SimpleStringProperty(cell.getValue().getFrom()));

        destinationCol.setCellValueFactory((cell) -> new SimpleStringProperty(cell.getValue().getDestination()));

        departureTimeCol
                .setCellValueFactory((cell) -> new SimpleStringProperty(getDepartureTimeString(cell.getValue())));

        arrivalTimeCol.setCellValueFactory((cell) -> new SimpleStringProperty(getArrivalTimeString(cell.getValue())));
